# Iterativo final para profe
##############################################################################################
##############################################################################################
################################## ALGORITMO GRG ITERATIVO ###################################
##############################################################################################
##############################################################################################
...
def funcionsuma():
    ...
    global RO
    RO = float(txt17.get())
    global ultimo_valor_ro
    ultimo_valor_ro = float(txt18.get())
    global paso_algortimo
    paso_algortimo = float(txt19.get())
    ventana.destroy()

ventana = Tk()
ventana.title("INGRESO DE DATOS ALGORITMO GRG ITERATIVO")
ventana.geometry("1080x800")
ventana.config(bg="DarkOliveGreen1")
...
lbl_titulo_1 = Label(ventana, text="ALGORITMO GRG ITERATIVO", font=("Cambia Math", "20", "bold")).place(x=1040, y=70)
...
lbl_titulo_4 = Label(ventana, text="Parametros de Ingreso del Vector Factor Optimizante:", font=("Cambia Math", "11", "bold")).place(x=1060, y=360)
################################################################################
################################################################################

lbl17 = Label(ventana, text = "Ingrese el valor inicial del factor" "\n" "optimizante, recomendación: ingrese el cero")
lbl17.place(x=1050, y=400, width = 250, height = 30)
txt17 = Entry(ventana, bg="yellow")
txt17.place(x=1350, y = 400, width=100, height=30)
txt17.config(font=("Calibri", "12"))

lbl18 = Label(ventana, text = "Ingrese el valor final del factor" "\n" "optimizante, recomendación: ingrese el 10")
lbl18.place(x=1050, y=450, width = 250, height = 40)
txt18 = Entry(ventana, bg="yellow")
txt18.place(x=1350, y = 450, width=100, height=30)
txt18.config(font=("Calibri", "12"))

lbl19 = Label(ventana, text = "Ingrese el paso del vector factores" "\n" "optimizantes, recomendación: ingrese 0.1")
lbl19.place(x=1050, y=500, width = 250, height = 40)
txt19 = Entry(ventana, bg="yellow")
txt19.place(x=1350, y = 500, width=100, height=30)
...
#######################################################################################################################
################## REGRESION LINEAL LEY MEDIA MAS ALLA DE LA ULTIMA MARCA DE CLASE DE LEYES DE CORTE ##################
#######################################################################################################################
...
# instruimos a la regresión lineal que aprenda de los datos (ahora polinómicos DE GRADO 1, LINEAL) (X,y)
...
####################################################################################################
################################ FINAL DE LAS REGRESIONES LINEALES #################################
####################################################################################################
...
tonqueda = 100  ### SE COLOCA ESTE VALOR, SOLO PARA QUE PUEDA ENTRAR AL CICLO WHILE !!!!!

while RO <= ultimo_valor_ro :    #ACA SE DEBE INGRESAR HASTA DONDE QUEREMOS LLEGAR CON EL VALOR DE RO !!!!!!!!!!!!!!!!!!!!!!
  while leyes["mineral"].sum()>0 and tonqueda>0: 
    ...
    while error>0.01 and iii<=160:        #ACA VA EL WHILE 
      ...
      gh_lane = (c+RO+h_reha+((f+(VAN_0*d))/C))/((s-r)*y)  # LEY DE CORTE CONCENTRADORA CON LANE Y RECORDAR QUE EL RO DEBE LLEGAR HASTA 1 Y PASO DE 1
      ...
      leyesorden
      ### SE DEFINE LA LEY DE CORTE OPTIMA COMO AQUELLA DADA SOLO POR LA PLANTA CONCENTRADORA 
      Goptimo = float(gh)
      ...
      if 1 in beta: 
        ...
        if años >=1: #sino funciona probar con añosvan   
          qm = Qm/años 
          qc = Qc/años 
          qr = qc*y*ley_media 
          if qr>R: 
      ...
      ######################################################################################
        ...
        if años >1: #sino funciona probar con añosvan   
          ...
          if qr>R: ### SI EXISTE CONCENTRADO Y SE VENDE 
            ...
            qconc = 0 ## NO EXISTE CONCENTRADO QUE PUEDA VENDERSE 
            print("EL TONELAJE DE CONCENTRADO ES: ",qconc)
            print("el valor final de qm es",qm, "el de qc es:",qc,"y el de qr es", qr) 
            if respuesta == "SI" or respuesta == "si":
              b_lane = ((s-r)*qr)+(pconc*qconc)-(c+m-e-h_reha)*qc-(e+h_reha)*qm-(f*(qc/C))  ### CON ECONOMIAS DE ESCALAS
...
ARCHIVADOR_SELECCION_2 = filtrado_archivador_maximo
...
print("#################################################################################         GRG          #############")
print("####################################################################################   ITERATIVO    ################")
print("############# RESULTADOS FINALES DEL ALGORITMO GRG ITERATIVO #########################            ##################")

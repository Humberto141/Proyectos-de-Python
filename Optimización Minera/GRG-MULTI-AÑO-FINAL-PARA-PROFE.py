# GRG ESPERANZA MULTI AÑO PARA PROFE
##############################################################################################
##############################################################################################
##################### ALGORITMO GRG MULTI AÑO CON SELECCION DE PLAN ##########################
##############################################################################################
##############################################################################################
...
def funcionsuma():
    ...
    global VALOR_INICIAL_FACTOR_OPTIMIZANTES
    VALOR_INICIAL_FACTOR_OPTIMIZANTES = float(txt17.get())
    global ULTIMO_VALOR_VECTOR_FACTORES_OPTIMIZANTES
    ULTIMO_VALOR_VECTOR_FACTORES_OPTIMIZANTES = float(txt18.get())
    global PASO_VECTOR_FACTORES_OPTIMIZANTES
    PASO_VECTOR_FACTORES_OPTIMIZANTES = float(txt19.get())
    global SELECCION_PLAN
    SELECCION_PLAN = str(txt20.get())
    ventana.destroy()

ventana = Tk()
ventana.title("INGRESO DE DATOS ALGORITMO GRG MULTI AÑO")
ventana.geometry("1080x800")
ventana.config(bg="aquamarine2")

####### CREACION DE LABELS Y CUADRITOS QUE ALMACENAN LA INFORMACIÓN #######
...
########################### INGRESO DE LA DISTRIBUCION DE LEYES ##############################
...
############################### INGRESO DEL TONELAJE DE MINERAL ###################################
...
######################## INFORMACIÓN IMPORTTANTE SOBRE EL INGRESO DE DATOS A LA GUI #############################

lbl_titulo_1 = Label(ventana, text="ALGORITMO GRG MULTI AÑO", font=("Cambia Math", "20", "bold")).place(x=1050, y=40)
lbl_titulo_2 = Label(ventana, text="Consideraciones sobre el Ingreso de Datos:", font=("Cambia Math", "14", "bold")).place(x=1040, y=90)
...
"de porcentaje, es decir, solo ingresar el numero." "\n"
"Podra escoger el tipo de plan minero que desee, si desea" "\n"
"encontrar aquel plan o solucion que maximice el VAN debe" "\n"
"seleccionar la letra A, en cambio, si desea buscar aquel plan" "\n"
"o solucion que reporte el maximo VAN pero manteniendo el movimiento" "\n"
"mina constante a lo largo de la vida del proyecto, seleccione la letra B", justify ="center", font=("Cambia Math", "11", "bold")).place(x=990, y=130)

lbl_titulo_4 = Label(ventana, text="Parametros de Ingreso del Vector Factor Optimizante:", font=("Cambia Math", "11", "bold")).place(x=1060, y=440)
################################################################################
################################################################################

lbl17 = Label(ventana, text = "Ingrese el valor inicial del factor" "\n" "optimizante, recomendación: ingrese el cero")
lbl17.place(x=1050, y=480, width = 250, height = 30)
txt17 = Entry(ventana, bg="yellow")
txt17.place(x=1350, y = 480, width=100, height=30)
txt17.config(font=("Calibri", "12"))

lbl18 = Label(ventana, text = "Ingrese el valor final del factor" "\n" "optimizante, recomendación: ingrese el 30")
lbl18.place(x=1050, y=530, width = 250, height = 40)
txt18 = Entry(ventana, bg="yellow")
txt18.place(x=1350, y = 530, width=100, height=30)
txt18.config(font=("Calibri", "12"))

lbl19 = Label(ventana, text = "Ingrese el paso del vector factores" "\n" "optimizantes, recomendación: ingrese 0.1 o 0.01")
lbl19.place(x=1045, y=580, width = 260, height = 40)
txt19 = Entry(ventana, bg="yellow")
txt19.place(x=1350, y = 580, width=100, height=30)
txt19.config(font=("Calibri", "12"))

lbl20 = Label(ventana, text = "INGRESE EL TIPO DE PLAN QUE DESEE:" "\n" "si desea maximizar el VAN ingrese la letra A" "\n" "si desea el movimiento mina constante, ingrese la letra B")
lbl20.place(x=1020, y=630, width = 300, height = 50)
txt20 = Entry(ventana, bg="yellow")
txt20.place(x=1350, y = 630, width=100, height=30)
txt20.config(font=("Calibri", "12"))

############## BOTON FINAL PARA ACEPTAR LOS DATOS INGRESADOS AL SISTEMA #################
...
################ TRANFORMACION DE UNIDADES DE ALGUNOS PARAMETROS ECONOMICOS INGRESADOS ###############
...
print("el costo de extraccion del esteril es: ", e)
print("la capacidad de la refineria es de: ", R)
print("la capacidad de la planta es de: ", CAPACIDAD_PLANTA)
print("la ley del concentrado es de: ", ley_conc)
print("la recuerpacion metalurgica es de: ", perdida_metalurgica)


############ LLEVAR LOS DATOS DE LA DISTRIBUCIONES A LISTAS ############
...
base_de_datos = {"leymin":leymin, "leymax":leymax, "mineral": mineral}  ####DESACTIVAR SI ESTAMOS EN GOOGLE COLAB
leyes = pd.DataFrame(data = base_de_datos) #### DESACTIVAR SI ESTAMOS EN COLAB DE GOOGLE
...
##############################################################################
#################### DEFINCION DE ECUACIONES ESPECIALES ######################
##############################################################################
...
###########################################################################################################################
######### REGRESION LINEAL PARA HALLAR LA LEY MEDIA CUANDO LA LEY DE CORTE ES MAYOR A LA ULTIMA MARCA DE CLASE  ###########
###########################################################################################################################
...
######################################################################## ## ### #### #### ########### ####### ######## ############
############# TERMINO DE DEFINICIONES DE FUNCIONES ESPECIALES ########### ## ### ### ### ############# ###### ####### #############
########################################################################## ## ### ## ## ############### ##### ###### ##############
########################################################################### ## ### # # ################# #### ##### ###############
...
pendiente_leymedia_mas_alla = regresion_lineal_leymedia_mas_alla.coef_[1]
...
archivador_años_operativos = []
archivador_vanes_operativos = []
archivador_ro_operativo = []
archivador_leycorte_operativo = []
archivador_Qm_operativo = []
archivador_Qc_operativo = []
archivador_Qr_operativo = []
archivador_leymedia_operativo = []
archivador_beneficio_operativo = []
archivador_conc_operativo = []
archivador_tiempoespecial_operativo = []
...
while leyes["mineral"].sum()>0 and tonqueda>0: 
  ...
  ##################### REGRESION POLINOMICA PARA LA RAZON E/M ######################
  ...
  VAN_0 = 0 ##### SE INICIA CON UN VAN INICIAL IGUAL A 0
  error = 10000000 #### SE DEFINE ESTE ERROR PARA QUE PUEDA ENTRAR DENTRO DEL CICLO WHILE 
  iii = 1 ## CONTADOR PARA LAS ITERACIONES DENTRO DE CADA AÑO ######
  ########## lineas de cambio ################

  PASO_RO = PASO_VECTOR_FACTORES_OPTIMIZANTES
  ULTIMO_RO = ULTIMO_VALOR_VECTOR_FACTORES_OPTIMIZANTES
  RO = VALOR_INICIAL_FACTOR_OPTIMIZANTES

  #########################################################
  while  RO <= ULTIMO_RO:        #ACA VA EL WHILE 
    print("\n")
    gm = (c)/((s-r)*y) #ley de corte mina 
    gh = (c+RO+((f+(VAN_0*d))/C))/((s-r)*y) #ley de corte de la concentradora 
    gk = (c)/(((s-r)-((f+(VAN_0*d))/R))*y)       #ley de corte de la refineria 
    gmh = 1 - (C/M)
    ghk = (2*R/C)-1
    gkm = pow((1-((2*R)/M)),0.5) 
    if respuesta == "SI" or respuesta == "si": 
      g_operativa = (c+m-e+RO-h_reha)/((s-r)*y)  ### CON ECONOMIAS DE ESCALAS
    if respuesta == "NO" or respuesta == "no":
      g_operativa = (c+RO-h_reha)/((s-r)*y)  ### SIN ECONOMIAS DE ESCALAS 
    print("LA LEY DE CORTE OPERATIVA ES: ", g_operativa)
    print("LA LEY DE CORTE OPERATIVA ES: ", g_operativa*100, "%")
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    ...
    #linea para establecer el G optimo a partir de la mediana de los tres valores obtenidos de leyes 
    leyesxxx = pd.DataFrame(data=leyesxx)
    leyesxxx
    leyesorden=leyesxxx.sort_values(by="leyes")
    leyesorden
    Goptimo = float(g_operativa)
    print("EL VALOR DE GOPTIMO ES:",Goptimo)
    print("ESTAMOS EN EL AÑO: ", anio)
    print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")

    ############### LINEA SI EL GOPTIMO ES IGUAL A UN VALOR EXACTO O NO ##############
    ...
    if 1 in beta: ######### TONELAJES HOMOGENEOS ##########
      ...
      print("EL VALOR DE LA LEY MEDIA ES: ", ley_media)
...
######################################################
      ...
      if años<1: 
        ...
        if qr>R: ### SI EXISTE CONCENTRADO Y SE VENDE 
          qdelta = qr-R 
          qconc = (qdelta)/(ley_conc*FURE)
          qr = R 
          print("LA CANTIDAD DE CONCENTRADO ES: ", qconc)
...
######################################################
    ...
    ######################################################################################
      ...
      if Goptimo > leyes.iloc[-1,0]:                           ######## LINEAS DE CODIGO POR SI LA LEY DE CORTE ENCONTRADA 
        ...
        if Goptimo < leyes.iloc[0,3]:                                            ### LINEAS DE CODIGO CUANDO LA LEY DE CORTE HALLADO ES MENOR
          ...
          # CUANDO BUSCAMOS UNA LEY MEDIA PARA CUALQUIER LEY DE CORTE, LA "ley_media" ES EL PROMEDIO DE LA ###
          # LEY DE CORTE SUPERIOR Y LA INFERIOR DENTRO DE LA TABLA "leyes"
          ...
          # LA "ley_media2" CORRESPONDE A LA INTERPOLACION LINEAL PARA HALLAR LA LEY MEDIA SEGUN CUALQUIER LEY DE CORTE ##
          ...
          # LA "ley_media_polinomica" ES LA REGRESION POLINOMICA DE LA LEY MEDIA, SEGUN LOS DATOS ESTABLECIDOS EN LA TABLA "leyes"
          ...
          # SE CALCULARA LA LEY MEDIA COMO LA INTERPOLACION LINEAL ENTRE LA LEY MEDIA SUPERIOR E INFERIOR SEGUN LA ##
          # LEY DE CORTE BUSCADA 
          Qr = Qc*y*ley_media2
          # EL Qm SE DEFINE COMO LA CANTIDAD DE MATERIAL TOTAL EN UN DETERMINADO PERIODO
          Qm = leyes["mineral"].sum()
      ...
      #### LIMITANTE A MI SISTEMA                                                                 #####
      #################################################################################################
      años = float(tc)  #ESTE VALOR ES EL VERDADERO VALOR DE AÑOS
      decimal = años - int(años)
    ...
    iii = iii + 1
    archivador_años_operativos.append(anio)
    archivador_vanes_operativos.append(VAN) 
    archivador_ro_operativo.append(RO)
    archivador_leycorte_operativo.append(Goptimo*100)
    archivador_Qm_operativo.append(qm)
    archivador_Qc_operativo.append(qc)
    archivador_Qr_operativo.append(qr)
    archivador_conc_operativo.append(qconc)
    archivador_tiempoespecial_operativo.append(años)
    if Goptimo < leyes.iloc[0,3]:
      archivador_leymedia_operativo.append(leymedia_MAS_BAJA*100)
    else: 
      archivador_leymedia_operativo.append(ley_media2*100)

    archivador_beneficio_operativo.append(b_lane)
    VAN_0 = VAN
    RO = RO + PASO_RO

######################################################""""""""####""#"#"#"#"#

  if SELECCION_PLAN == "A" or SELECCION_PLAN == "a":
    datossalida_operativos  = {"Años":archivador_años_operativos,"ro": archivador_ro_operativo, "VAN": archivador_vanes_operativos , "Ley de Corte en %": archivador_leycorte_operativo , "beneficio anual": archivador_beneficio_operativo,  "ley media en %": archivador_leymedia_operativo, "Ton de Material": archivador_Qm_operativo , "Ton de Mineral": archivador_Qc_operativo , "Ton de Producto": archivador_Qr_operativo, "Ton de Concentrado": archivador_conc_operativo, "tiempo especial": archivador_tiempoespecial_operativo} 
    archivador_final_ = pd.DataFrame(data = datossalida_operativos) 
    SELECCION_AÑOS = archivador_final_["Años"] == anio   ### LINEA DE CODIGO QUE SELECCIONA LAS FILAS EN EL AÑO EN EL QUE ESTAMOS
    filtrado_por_años = archivador_final_[SELECCION_AÑOS]
    print(filtrado_por_años)   ### MUESTRA UN DATAFRAME SOLO CON LAS FILAS DEL AÑO EN EL QUE NOS ENCONTRAMOS
    maximo_van_por_año = filtrado_por_años["VAN"].max()  ## LINEA QUE GUARDA EL MAXIMO VAN PARA UN DETERMINADO AÑO
    fila_maximo_van_por_año = filtrado_por_años.loc[filtrado_por_años["VAN"] == maximo_van_por_año]   ### DE ESE DATAFRAME, SELECCIONAMOS LA FILA QUE MUESTRA SOLO EL VAN MAXIMO 
    qm_extraido = fila_maximo_van_por_año.iloc[0,6]   #### obtenemos la cantidad de material extraido en el año i (anio)
    print("EL VALOR DEL TONELAJE EXTRAIDO EN EL AÑO", anio, "ES DE", qm_extraido)
    reparacion.append(qm_extraido)
  ##############################################
  ##### ACTIVAR SOLO CUANDO EL PRECIO DEL COBRE SEA MENOR A 2 cUSD/lb 
  ######666666666666666666666666666666666666666666666
  ######666666666666666666666666666666666666666666666
  ######666666666666666666666666666666666666666666666
  #  if anio >1:
  #    respetar_tonelaje = reparacion[-1]
  #    respetar_tonelaje_base = archivador_final_.loc[archivador_final_["Ton de Material"] <= respetar_tonelaje]
  #    archivador_final_ = respetar_tonelaje_base
  ######666666666666666666666666666666666666666666666
  ######666666666666666666666666666666666666666666666
  ######666666666666666666666666666666666666666666666
    MAXIMO_VAN_OPERATIVO = archivador_final_["VAN"].max()
    fila_maxima_operativa = archivador_final_.loc[archivador_final_["VAN"] == MAXIMO_VAN_OPERATIVO]
    print(fila_maxima_operativa)

  if SELECCION_PLAN == "B" or SELECCION_PLAN == "b":
    datossalida_operativos  = {"Años":archivador_años_operativos,"ro": archivador_ro_operativo, "VAN": archivador_vanes_operativos , "Ley de Corte en %": archivador_leycorte_operativo , "beneficio anual": archivador_beneficio_operativo,  "ley media en %": archivador_leymedia_operativo, "Ton de Material": archivador_Qm_operativo , "Ton de Mineral": archivador_Qc_operativo , "Ton de Producto": archivador_Qr_operativo, "Ton de Concentrado": archivador_conc_operativo, "tiempo especial": archivador_tiempoespecial_operativo} 
    archivador_final_ = pd.DataFrame(data = datossalida_operativos) 
    if anio == 1: 
      SELECCION_AÑOS = archivador_final_["Años"] == anio   ### LINEA DE CODIGO QUE SELECCIONA LAS FILAS EN EL AÑO EN EL QUE ESTAMOS
    ...
    if anio > 1: 
      ...
      filtrado_por_años = archivador_final_[SELECCION_AÑOS]
      print(filtrado_por_años)   ### MUESTRA UN DATAFRAME SOLO CON LAS FILAS DEL AÑO EN EL QUE NOS ENCONTRAMOS
      fila_ton_snake_por_año = filtrado_por_años.loc[filtrado_por_años["Ton de Material"] <= ton_snake ]
      ton_snake_maximo = fila_ton_snake_por_año["Ton de Material"].max()
      qm_extraido = ton_snake_maximo




  #  qm_extraido = fila_maxima_operativa.iloc[0,6]  ####666666666666666666666

#  plt.plot(archivador_ro_operativo, archivador_vanes_operativos, color='red', marker='o', label = "X", linestyle='dashed',linewidth=1, markersize=10)
#  plt.grid(linewidth = 1)
#  plt.axis([0,30,0,2000000000])
#  plt.title("VANES vs RO OPTIMO ")
#  plt.xlabel("RO OPTIMO")
#  plt.ylabel("VAN usd")
#  plt.show()
...
#  qm_max = fila_maxima_operativa.iloc[0,6]
  qm_max = qm_extraido
...
print("EL ARCHIVADOR FINAL TAN TAN ES: ")
ingreso = archivador_final_ #### es el dataframe que contiene toda la informacion resultante del cut off grade optimizer 
años_totales = anio - 1 
print("LA VIDA DE LA MINA ES DE: ", años_totales)
...
if SELECCION_PLAN == "A" or SELECCION_PLAN == "a": 
  for i in range(1, años_totales + 1):
    año1 = ingreso["Años"] == i
    filtrado_año1 = ingreso[año1]
    print(filtrado_año1)

    max_van = filtrado_año1["VAN"].max()

    resultado_van_maximo = filtrado_año1.loc[filtrado_año1["VAN"] == max_van]
    print(resultado_van_maximo)

    años_tt1.append((resultado_van_maximo.iloc[0,0]))
    ro_tt1.append((resultado_van_maximo.iloc[0,1]))
    van_tt1.append((resultado_van_maximo.iloc[0,2]))
    cog_tt1.append((resultado_van_maximo.iloc[0,3]))
    beneficio_tt1.append((resultado_van_maximo.iloc[0,4]))
    leymedia_tt1.append((resultado_van_maximo.iloc[0,5]))
    qm_tt1.append((resultado_van_maximo.iloc[0,6]))
    qc_tt1.append((resultado_van_maximo.iloc[0,7]))
    qr_tt1.append((resultado_van_maximo.iloc[0,8]))
    qconc_tt1.append((resultado_van_maximo.iloc[0,9]))
...
if SELECCION_PLAN == "B" or SELECCION_PLAN == "b":
  for i in range(1, años_totales + 1):
    if i == 1: 
      ...
      resultado_van_maximo = filtrado_año1.loc[filtrado_año1["VAN"] == max_van]
      print(resultado_van_maximo)
      tonraiden = resultado_van_maximo.iloc[0,6]
      print("el tonelaje raiden es:", tonraiden)

    if i >1: 
      año1 = ingreso["Años"] == i
      filtrado_año1 = ingreso[año1]
      print(filtrado_año1)
      resultado_van_maximo = filtrado_año1.loc[filtrado_año1["Ton de Material"] <= tonraiden]
      ton_raiden_maximo = resultado_van_maximo["Ton de Material"].max()
      resultado_van_maximo = resultado_van_maximo.loc[resultado_van_maximo["Ton de Material"] == ton_raiden_maximo]
      print(resultado_van_maximo)
...
wolverine  = {"Años":años_tt1,"ro": ro_tt1, "VAN": van_tt1 , "Ley de Corte en %": cog_tt1 , "beneficio anual": beneficio_tt1,  "ley media en %": leymedia_tt1, "Ton de Material": qm_tt1 , "Ton de Mineral": qc_tt1 , "Ton de Producto": qr_tt1, "Ton de Concentrado": qconc_tt1} 
print(wolverine)

print("&&&&&&&&&&&&&&&&&&&&")

wolverine2 = pd.DataFrame(data = wolverine) 
print(wolverine2) 

ARCHIVADOR_SELECCION_2 = wolverine2
...
print("#################################################################################         GRG          #############")
print("####################################################################################   MULTI AÑO    ################")
print("############# RESULTADOS FINALES DEL ALGORITMO GRG MULTI AÑO #########################            ##################")
print("##################### SEPARADOR DECIMAL ES EL PUNTO #################################      ##      #################")
print("#####################################################################################   ########   #################")
print("####################################################################################  ############  ################")

lista_1 = {"Año": np.array(ARCHIVADOR_SELECCION_2["Años"]), "VAN MUSD": np.array(ARCHIVADOR_SELECCION_2["VAN verdadero"]/1000000), "Beneficio anual MUSD": np.array(ARCHIVADOR_SELECCION_2["beneficio anual"]/1000000) , "Ley de Corte en %": np.array(ARCHIVADOR_SELECCION_2["Ley de Corte en %"]) , "ley media en %": np.array(ARCHIVADOR_SELECCION_2["ley media en %"])}
lista_a = pd.DataFrame(data = lista_1)
print(lista_a)
lista_3 = {"Año": np.array(ARCHIVADOR_SELECCION_2["Años"]), "Material Extraido en Mton": np.array(ARCHIVADOR_SELECCION_2["Ton de Material"]/1000000), "Mineral a Planta en Mton": np.array(ARCHIVADOR_SELECCION_2["Ton de Mineral"]/1000000), "Cobre Fino en Toneladas": np.array(ARCHIVADOR_SELECCION_2["Ton de Producto"]), "Concentrado en Toneladas": np.array(ARCHIVADOR_SELECCION_2["Ton de Concentrado"])}
...
#ARCHIVADOR_SELECCION_2.to_excel(titulo)  ###### CODIGO QUE HAY QUE ACTIVAR PARA COLAB (ENTREGA EL EXCEL) ######

 # datos_ultra_fase_2_final.to_excel("")
...
plt.bar(ARCHIVADOR_SELECCION_2["Años"], height = ARCHIVADOR_SELECCION_2["VAN verdadero"], width = 0.5)
plt.title("VAN vs años")
plt.xlabel("Años")
plt.ylabel("Van")
plt.show()

plt.plot(ARCHIVADOR_SELECCION_2["Años"], ARCHIVADOR_SELECCION_2["Ley de Corte en %"], color='red', marker='o', label = "X", linestyle='dashed',linewidth=1, markersize=10)
plt.grid(linewidth = 1)
plt.axis([0,float(anio+1),0,1])
plt.title("Goptimo vs Años")
plt.xlabel("Años")
plt.ylabel("ley de corte optima en %")
plt.show()

plt.bar(ARCHIVADOR_SELECCION_2["Años"], ARCHIVADOR_SELECCION_2["Ton de Material"], width = 0.5, color = "orange")
plt.title("Tonelaje de material en cada año")
plt.xlabel("Años")
plt.ylabel("Cantidad de material en toneladas")
plt.show()